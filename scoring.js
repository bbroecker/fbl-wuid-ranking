...
function initFirebase() {
    ...
    // Listen for Circle21 data changes
    window.database.ref('circle21').on('value', (snapshot) => {
        const data = snapshot.val();
        circle21Cache = data || [];
        
        // Update Circle21 displays
...
function getAllCircle21Athletes() {
    if (!database) {
        const stored = localStorage.getItem('fbl-circle21-athletes');
...
function saveAllCircle21Athletes(athletes) {
    localStorage.setItem('fbl-circle21-athletes', JSON.stringify(athletes));
    
    if (database) {
        database.ref('circle21').set(athletes);
    }
    
    circle21Cache = athletes;
}

// Save or update a Circle21 athlete
function saveCircle21Athlete() {
    ...
    if (!name || !overall) {
        showToast('Please fill in athlete name and overall placement', true);
        return;
    }
    ...
    const workouts = {};
    for (let i = 1; i <= 6; i++) {
        const placement = document.getElementById(`circle21-lq${i}`).value;
        if (placement) {
            workouts[`LQ${i}`] = parseInt(placement);
        }
    }
    ...
    const existingIndex = athletes.findIndex(a => 
        a.name.toLowerCase() === name.toLowerCase() && a.gender === gender
    );
    ...
    if (existingIndex >= 0) {
        ...
        showToast(`Updated ${name}'s placements`);
    } else {
...
function clearCircle21Form() {
    ...
    for (let i = 1; i <= 6; i++) {
        document.getElementById(`circle21-lq${i}`).value = '';
    }
}

// Load athlete into form for editing
function editCircle21Athlete(name, gender) {
    ...
    if (athlete) {
        ...
        for (let i = 1; i <= 6; i++) {
            const placement = athlete.workouts[`LQ${i}`];
            document.getElementById(`circle21-lq${i}`).value = placement || '';
        }
...
function deleteCircle21Athlete(name, gender) {
    ...
    saveAllCircle21Athletes(filtered);
    showToast(`Removed ${name} from Circle21 tracking`);
    displayCircle21AthletesList();
}

// Display Circle21 athletes list in admin
function displayCircle21AthletesList() {
    ...
    if (athletes.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: #999; padding: 40px;">No athletes added yet. Add athletes above to start tracking.</p>';
        return;
    }
    
    // Group by gender
    const male = athletes.filter(a => a.gender === 'M');
    const female = athletes.filter(a => a.gender === 'F');
    
    let html = '';
    
    if (male.length > 0) {
        html += '<h4 style="color: #008AC2; margin-bottom: 15px;">Male (M)</h4>';
            ...
            html += `
                <div class="circle21-athlete-card">
                    <div class="circle21-athlete-info">
                        ...
                        <div class="circle21-athlete-stats">Overall: #${athlete.overall} | Workouts: ${Object.keys(athlete.workouts).length}/6</div>
                    </div>
                    <div class="circle21-athlete-actions">
                        <button class="btn btn-edit" onclick="editCircle21Athlete('${athlete.name}', '${athlete.gender}')">Edit</button>
                        <button class="btn btn-secondary" onclick="deleteCircle21Athlete('${athlete.name}', '${athlete.gender}')">Delete</button>
                    </div>
                </div>
            `;
        });
    ...
    if (female.length > 0) {
        html += '<h4 style="color: #008AC2; margin-bottom: 15px; margin-top: 20px;">Female (F)</h4>';
...
function displayCircle21Leaderboard() {
    ...
    const gender = document.getElementById('circle21GenderFilter').value;
    const athletes = getAllCircle21Athletes().filter(a => a.gender === gender);
    
    if (athletes.length === 0) {
        container.innerHTML = '<p style="text-align: center; color: #999; padding: 60px;">No athletes tracked for this gender yet.</p>';
        return;
    }
    ...
    const sorted = [...athletes].sort((a, b) => {
        ...
        if (circle21SortColumn === 'name') {
            valA = a.name.toLowerCase();
            valB = b.name.toLowerCase();
            return circle21SortAscending ? 
                valA.localeCompare(valB) : 
                valB.localeCompare(valA);
        } else if (circle21SortColumn === 'overall') {
            valA = a.overall;
            valB = b.overall;
        } else {
            // Workout column
            valA = a.workouts[circle21SortColumn] || 999999;
            valB = b.workouts[circle21SortColumn] || 999999;
        }
        
        return circle21SortAscending ? valA - valB : valB - valA;
    });
    ...
    for (let i = 1; i <= 6; i++) {
        const col = `LQ${i}`;
        html += `<th onclick="sortCircle21('${col}')" style="cursor: pointer;">${col} ${circle21SortColumn === col ? (circle21SortAscending ? '▲' : '▼') : ''}</th>`;
    }
    ...
    sorted.forEach((athlete, index) => {
        ...
        html += `<tr class="${rowClass}">`;
        html += `<td>${athlete.overall}</td>`;
        html += `<td>${athlete.name}</td>`;
